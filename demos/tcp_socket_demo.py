...
tcp_seq = 454  # seuence number
tcp_ack_seq = 0  #tcp ack seq num
...
tcp_win = 65535  # maximum allowed window reordered to network order
...
tcp_hdr = struct.pack('!HHLLBBHHH', tcp_src, tcp_dst, tcp_seq, tcp_ack_seq, tcp_off_res, tcp_flags, tcp_win, tcp_chk, tcp_urg_ptr) # L means 32 bit word as an integer
...
tcp_hdr = struct.pack('!HHLLBBH', tcp_src, tcp_dst, tcp_seq, tcp_ack_seq, tcp_off_res, tcp_flags, tcp_win) + struct.pack('H', tcp_chk) + struct.pack ('!H', tcp_urg_ptr)
...
# don't need this right now because we are using this as a test rather than an actual connection to establish
# s.close()
